...
import com.example.pokemon.ui.navigation.CombatDestination
...
//    pokemonEntryViewModel: PokemonEntryViewModel,
    ...
    Scaffold(
        ...
        val pokemonEnnemyListUiState = pokemonEnnemyListViewModel.uiState.collectAsState()
        ...
        Column(
            ...
            Button(onClick = {
                coroutineScope.launch {
                    pokemonEnnemyListViewModel.reset()
                }
            }) {
                Text(text = stringResource(R.string.reset_ennemy_list))
            }
            CombatList(
                pokemonEnnemyListUiState.value.list,
                paddingValues = it,
                chooseCombat = { ennemy ->
                    navController.navigate(CombatDestination.route)
                    pokemonCombatViewModel.setEnnemy(ennemy)
                }
            )
        }
...
fun CombatList(
    ...
    LazyColumn(
        ...
        items(list) {
            CombatItem(pokemon = it, chooseCombat = {ennemy ->  chooseCombat(ennemy) })
        }
    }
    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                IconButton(onClick = { chooseCombat(pokemon) }) {
                    Icon(
